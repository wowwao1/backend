...
{	
	...
	{
		if(ChkVar($verify_action) && ($verify_action == 'Add' || $verify_action == 'Edit'))
		{
			if(ChkVar($user_id))
			{
				if(ChkVar($phone_no))
			...
			{
				APIerror('User Id not found');
			}
		}
		else if(ChkVar($verify_action) && $verify_action == 'Update')
		{
			if(ChkVar($user_id))
			{
				if(ChkVar($phone_code))
				{
					if(ChkVar($phone_no))
					{	
		...
		{
			APIerror('Invalid Phone Varification Action');
		}
	}
	...
	{
		if(ChkVar($user_id) && ChkVar($firstname) && ChkVar($lastname) && ChkVar($gender) && ($gender == 'Male' || $gender == 'Female') && ChkVar($dob) && ChkVar($city) && ChkVar($state) && ChkVar($country) && ChkVar($city_lat) && ChkVar($city_long))
		{
